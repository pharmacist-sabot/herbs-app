...
// Lifecycle
onMounted(() => {
    fetchHerb(route.params.id);
});

// Watch for route changes (e.g., navigating from one herb to another)
watch(
    () => route.params.id,
    (newId) => {
        if (newId) fetchHerb(newId);
    }
);

// Mock Data Function (Moved logic here to keep setup clean)
const getMockHerb = (id) => {
    // Cast ID strictly for comparison if needed
